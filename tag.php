...
$tag_name = (is_a($current_tag, 'WP_Term')) ? ucwords(strtolower($current_tag->name)) : 'Tag';
$tag_desc = (is_a($current_tag, 'WP_Term')) ? $current_tag->description : '';
...
<main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 flex flex-col gap-8">
  ...
  <header class="mb-8 border-b border-gray-100 dark:border-gray-800 pb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
    <div class="flex-1">
      ...
      <h1 class="text-4xl md:text-5xl lg:text-6xl font-black leading-tight tracking-tight text-[#121617] dark:text-white capitalize">
        <span class="text-primary opacity-50 text-2xl md:text-3xl lg:text-4xl block mb-2">Tag:</span>
        <?php echo esc_html($tag_name); ?>
      </h1>
      <?php if ($tag_desc) : ?>
      <p class="mt-4 text-lg text-gray-500 dark:text-gray-400 max-w-2xl">
        <?php echo esc_html($tag_desc); ?>
      </p>
      <?php endif; ?>
    </div>
  ...
  <section class="mb-20">
    ...
    <div class="text-center py-20 bg-gray-50 dark:bg-gray-800/50 rounded-2xl">
      <span class="material-symbols-outlined text-4xl text-gray-300 mb-4">tag</span>
      <p class="text-gray-500 dark:text-gray-400 font-medium">No posts found with this tag yet.</p>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const currentView = localStorage.getItem('tag-view') || 'grid';
  if (currentView === 'list') {
    enableListView();
  }
  ...
  function enableGridView() {
    ...
    localStorage.setItem('tag-view', 'grid');
  }
  ...
  function enableListView() {
    ...
    localStorage.setItem('tag-view', 'list');
  }
});
</script>
